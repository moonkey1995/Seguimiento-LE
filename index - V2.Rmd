...
data<-read_excel(paste0(ruta,"/Seguimiento LE.xlsx"),
                 col_types = c("text","text","date","text","numeric","numeric","numeric")) %>% 
  mutate(`Start Time`=as_hms(`Start Time`* 86400),`End Time`=as_hms(`End Time`* 86400))#%>% 
...
Diario<-filtro %>% select(Date,`Main Title`,`rat#`,Inicio,rat_prev,`variación`) %>% arrange(Date) %>%  mutate(Date=as.character.Date(Date))
Promedios_F<-Promedios %>% mutate(`Main Title`="Promedio",`rat#`=Rat_miles,rat_prev=NA,`variación`=NA)%>% select(Date,`Main Title`,`rat#`,Inicio,rat_prev,`variación`)
...
P<-plot_ly(FINAL[-1,],x=~Date,y=~`rat#`, color=~`Main Title`,colores=c('forestgreen','orangered',"darkmagenta") , type = 'scatter',mode="lines-markers",hoverinfo = 'text',
                   text = ~paste('</br> Programa ', `Main Title`,
                                 '</br> Fecha ', Date,
                                 '</br> Rat miles: ', round(`rat#`,0),
                                 '</br> Variación: ', round(`variación`,1),"%"))%>% add_trace(y=~`rat#`,  mode = 'markers',symbols="square",color~ `Main Title`) %>% 
...
plot_ly(BaseJunta,x=~Date,y=~`rat#`, color=~`Inicio` , type = 'scatter',mode="lines-markers",hoverinfo = 'text',
                   text = ~paste('</br> Programa ', `Main Title`,
                                 ...
                                 '</br> Fecha ', Date,
                                 '</br> Rat miles: ', round(`rat#`,0),
                                 '</br> Variación: ', round(`variación`,1),"%"))%>% add_trace(y=~`rat#`,  mode = 'markers',symbols="square",color~ `Inicio`) %>% 
